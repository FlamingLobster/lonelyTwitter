...
public class ElasticsearchTweetController {
    ...
    public static class GetTweetsTask extends AsyncTask<String, Void, ArrayList<NormalTweet>> {
        ...
        protected ArrayList<NormalTweet> doInBackground(String... search_parameters) {
            ...
            if(search_parameters[0].isEmpty()) {
                search = new Search.Builder(search_parameters[0])
                        .addIndex("testing").addType("tweet").build();
            } else {
                search = new Search.Builder("{ \"query\": { \"term\": { \"message\" : \""+search_parameters[0]+"\" } } }")
                        .addIndex("testing").addType("tweet").build();
                System.out.println("{ \"query\": { \"term\": { \"message\" : \""+search_parameters[0]+"\" } } }");
            }
            try {
